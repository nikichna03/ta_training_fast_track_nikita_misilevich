...
namespace coordinates
{
    class Program
    {
        ...
        {
            Auto auto = new Auto(0, 0, 9.9, 0, "***", "***", 9.9, 9.9, true, "***");
            ...
            Chassis[] avto = { auto, truck, bus, Scooter };
            ...
            for (int i = 0; i < avto.Length; i++)
            {
                if (avto[i].size >= 1.5)
                {
                    Print(avto[i], i, xmlDoc, rootNode);
            ...
            for (int i = 2; i < 3; i++)
            {
                Print(avto[i], i, xmlDoc1, rootNode1);
            }
            ...
            for (int i = 0; i < avto.Length; i++)
            {
                Print(avto[i], i, xmlDoc2, rootNode2);
            }
            xmlDoc2.Save("XMLFile2.xml");
        }
        public static void Print(Шасси avto, int i, XmlDocument xmlDoc, XmlNode rootNode)
        {
